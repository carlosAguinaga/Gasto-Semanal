...
btn_calcular.onclick = ()=> {
    ...
    // etiqueta_li.innerHTML = `<div class="div_item_list"><span>${nombre_gasto.value}</span><span id="lista_monto" class="badge-pill badge-primary"> S/. <span>${cantidad_gasto.value}</span></span><cc>eeer</cc></div>`
    etiqueta_li.innerHTML = 
    ...
    etiqueta_li.getElementsByTagName('div')[0].getElementsByTagName('div')[0].getElementsByTagName('span')[2].addEventListener('click', editItem)
    etiqueta_li.getElementsByTagName('div')[0].getElementsByTagName('div')[0].getElementsByTagName('span')[3].addEventListener('click', deleteItem)
...
function actualizar_cantidades() {
    ...
    for (const item of arreglo_item_list) {
        let temp_price = item.getElementsByTagName('div')[0].getElementsByTagName('span')[1].getElementsByTagName('span')[0].innerHTML
